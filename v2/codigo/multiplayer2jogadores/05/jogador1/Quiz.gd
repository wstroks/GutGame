...
func set_question():
	...
	if childAmount >0:
		...
		var nome=str(multi_player._vezesta())
		get_node("feliz").hide()
		...
		get_node("Nome").set_text(nome)
	else:
		#questionText.set_text("Você acertou " + str(score) + ", Vamos agora pro feedback das questões?!")
		if(score>=3):
			multi_player._player1("Intestino Grosso",score)
			multi_player.tabuleiro1(6)
			multi_player._playervai1("res://codigo/multiplayer2jogadores/06/jogador1/Quiz.tscn")
			get_tree().change_scene(str(multi_player._playerondevai2()))
			multi_player.vez("Jogador 2")
		else:
			multi_player._player1("Intestino Delgado",score)
			multi_player.tabuleiro1(5)
			multi_player._playervai1("res://codigo/multiplayer2jogadores/05/jogador1/Quiz.tscn")
			get_tree().change_scene(str(multi_player._playerondevai2()))
			multi_player.vez("Jogador 2")
