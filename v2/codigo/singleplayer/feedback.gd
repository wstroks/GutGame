...
func _ready():
	...
	if(score>=3):
		...
		get_node("Label").set_text("Parábens você conseguiu avançar e agora vamos para o Estômago") 
...
func _on_Seguir_pressed():
	if(b==0):
		...
		get_node("Label").set_text("Agora estamos avançando para o Estômago, preparado?!")
		get_node("seta2").show()
...
func _on_jogarNovamente3_pressed():
	if(a==0):
		...
		get_node("Label2").hide()
		get_node("TextureRect2").hide()
		get_node("Label").set_text("Você ainda está na mesma posição, que é a Boca e o Esôfago!")
