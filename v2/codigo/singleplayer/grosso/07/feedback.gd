...
func _on_Seguir_pressed():
	if(b==0):
		...
		get_node("Label").set_text("Agora estamos avançando para casa 8 do Intestino Grosso, preparado?!")
		get_node("seta2").show()
...
func _on_jogarNovamente3_pressed():
	if(a==0):
		...
		get_node("Label2").hide()
		get_node("TextureRect2").hide()
		get_node("Label").set_text("Você ainda está na mesma posição 7, que corresponde ao Intestino Grosso!")
