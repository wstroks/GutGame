...
func _on_Seguir_pressed():
	if(b==0):
		...
		get_node("Label").set_text("Agora estamos avançando para casa 4 do Intestino Delgado, preparado?!")
		get_node("seta2").show()
...
func _on_jogarNovamente3_pressed():
	if(a==0):
		...
		get_node("Label").set_text("Você ainda está na mesma posição 3, que corresponde ao Intestino Delgado!")
