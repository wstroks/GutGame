...
func _ready():
	...
	if(score>=3):
		...
		get_node("Label").set_text("Parábens você conseguiu avançar e agora vamos para o Bile") 
...
func _on_Seguir_pressed():
	if(b==0):
		...
		get_node("Label").set_text("Agora estamos avançando para o Bile, preparado?!")
		get_node("seta2").show()
		
	if(b==1):
		get_tree().change_scene("res://codigo/singleplayer/Bile/Quiz.tscn")
	b=1
...
func _on_jogarNovamente3_pressed():
	if(a==0):
		...
		get_node("Label").set_text("Você ainda está na mesma posição 1, que corresponde ao Figado!")
...
func _on_Seguir2_pressed():
	if(a==1):
		get_tree().change_scene("res://codigo/singleplayer/Figado/Quiz.gd")
