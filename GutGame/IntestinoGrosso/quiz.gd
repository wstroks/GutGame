...
func set_question():
	...
	if childAmount >0:
		...
		if(score>=(numero/2)):
			get_node("Label").set_text("Parabéns você completou o Sistema Digestório, e obteve informações a respeito por onde passa o alimento.")
			get_node("menu").show()
		else:
			get_node("Label").set_text("Você não passou de fase, acertou apenas quantidade de " + str(score)+" de "+ str(numero)+"  questões!! Tente de novo!! ")
			get_node("menu").show()
...
func _on_jogar_n_pressed():
	...
	get_tree().change_scene("res://IntestinoGrosso/quiz.tscn")
